...
<table border="0" cellspacing="1"  id="STable" style="width:800px;">
	...
	{
	?>
		<tr><td colspan="7" style="padding:10px;">暂无FTP账号</td></tr>
	<?php	
	}
	else
	{
	...
	?>
			...
			<?php } else {?>
			<a href="index.php?c=ftp&a=ftp_list&edit=<?php echo $val['ftp_name'];?>" class="button"><span class="pen icon"></span> 编辑</a>
			<a href="index.php?c=ftp&a=ftp_list&del=<?php echo $val['ftp_name'];?>" class="button" onclick="return confirm('确认删除FTP账号:<?php echo $val['ftp_name'];?>?');"><span class="cross icon"></span> 删除</a>
...
<li>增加ftp用户: amh ftp add [账号] [密码] [根目录] [上传速度限制] [下载速度限制] [上传比率值] [下载比率值] [文件数量] [容量] [连接并发数] [使用时间限制]</li>
<li>编辑ftp用户: amh ftp edit [账号] [-] [根目录] [上传速度限制] [下载速度限制] [下载比率值] [下载比率值] [文件数量] [容量] [连接并发数] [使用时间限制]
