...
模块名字搜索 <input type="text" class="input_text" name="module_name" style="width:251px"/>
<button type="submit" name="download_submit" class="primary button" style="_margin-bottom:15px;_margin-left:3px;"> 下载 </button> 
</form>
</div>
<table border="0" cellspacing="1"  id="STable" style="width:1000px;">
	...
	?>
			<tr>
				...
				<?php } else {?>
					<button type="button" class="primary button" onclick="return (confirm('确认下载：<?php echo $val['module_name'];?> 模块吗?') && (WindowLocation('/index.php?c=module&a=module_down&module_name=<?php echo $val['module_name'];?>&page=<?php echo $page;?>')) && (this.innerHTML=' 请稍等...') && (this.disabled=true))" > 下载 </button>
				<?php } ?>
				</td>
			</tr>
	<?php
