...
class AmysqlProcess {
	...
	function ProcessStart()
	{
		...
		$PV6 = !function_exists('get_magic_quotes_gpc') ? true:false;	// php6
		if($Config['Filter'])	// 开启过滤
		{
			// php5才进行过滤、php6默认有过滤
			(!$PV6 && !get_magic_quotes_gpc() && 
			($this -> filter($_GET, 'addslashes') && $this -> filter($_POST, 'addslashes') && $this -> filter($_COOKIE, 'addslashes') && $this -> filter($_FILES, 'addslashes')));	
		}
		else
		{	
			// php5与php6都可清除过滤
		    ( ($PV6 || (!$PV6 && get_magic_quotes_gpc()) ) &&
			($this -> filter($_GET, 'stripslashes') && $this -> filter($_POST, 'stripslashes') && $this -> filter($_COOKIE, 'stripslashes') && $this -> filter($_FILES, 'stripslashes')));	
		}
	...
	function filter(&$array, $function) 
	{
		...
		foreach ($array as $key => $value) 
			(is_array($value) && $this -> filter($value, $function)) || $array[$key] = $function($value);
		Return true;
	}
}
