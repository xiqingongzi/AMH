...
<table border="0" cellspacing="1"  id="STable" style="width:800px;">
	...
	?>
			...
			<font style="color:#8B8B8B">编辑 &nbsp;删除</font>
			<?php } else {?>
			<a href="index.php?c=index&a=ftp&edit=<?php echo $val['ftp_name'];?>">编辑</a>
			<a href="index.php?c=index&a=ftp&del=<?php echo $val['ftp_name'];?>" onclick="return confirm('确认删除FTP账号:<?php echo $val['ftp_name'];?>?');">删除</a>
			<?php } ?>
			</td>
			</tr>
	<?php
...
<h3>» SSH FTP</h3>
1) 有步骤提示操作: <br />
ssh执行命令: /root/amh/ftp <br />
然后选择对应的1~6的选项进行操作。<br />

2) 或直接操作: <br />
<ul>
<li>查看ftp列表: /root/amh/ftp list </li>
<li>增加ftp用户: /root/amh/ftp add [账号] [密码] [根目录] [上传总流量] [下载总流量] [上传速度] [下载速度] [文件数量] [容量] [连接并发数] [使用时间限制]</li>
<li>编辑ftp用户: /root/amh/ftp edit [账号] [-] [根目录] [上传总流量] [下载总流量] [上传速度] [下载速度] [文件数量] [容量] [连接并发数] [使用时间限制]
<li>更改ftp密码: /root/amh/ftp pass [账号] [密码]
<li>删除ftp用户: /root/amh/ftp del [账号]</li>
</ul>

温馨提示:<br />
增加或编辑账号忽略参某参数请填0。 <br />
例如: /root/amh/ftp add testftp testpass /home/wwwroot 0 0 100 <br />
