...
<table border="0" cellspacing="1"  id="STable" style="width:<?php echo isset($list_show) ? 'auto':'1111px';?>">
	...
	?>
			...
			<td>
			<a href="index.php?c=index&a=host&edit=<?php echo $val['host_domain'];?>" class="button"><span class="pen icon"></span>编辑</a>
			<a href="index.php?c=index&a=host&del=<?php echo $val['host_domain'];?>" class="button" onclick="return confirm('确认删除虚拟主机:<?php echo $val['host_domain'];?>?');"><span class="cross icon"></span>删除</a>
			</td>
			</tr>
	<?php
		}
...
<li>新增虚拟主机: amh host add [主标识域名] [绑定域名] [默认主页] [Rewrite规则] [404页面] [访问日志 on/off] [错误日志 on/off]</li>
<li>编辑虚拟主机: amh host edit [主标识域名] [绑定域名] [默认主页] [Rewrite规则] [404页面] [访问日志 on/off] [错误日志 on/off]</li>
...
例如: amh host add amysql.com amysql.com,www.amysql.com index.html,index.php 0 0 0 on off<br />
...
2) 或直接操作: <br />
<ul>
<li>启动PHP: amh php start [主标识域名] 缺省主标识域名即为所有</li>
<li>停止PHP: amh php stop [主标识域名] 缺省主标识域名即为所有</li>
<li>重载PHP: amh php reload [主标识域名] 缺省主标识域名即为所有</li>
</ul>
</div>
