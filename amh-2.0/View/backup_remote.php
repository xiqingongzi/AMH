...
<table border="0" cellspacing="1"  id="STable" style="width:1000px;">
	...
	{
	?>
		<tr><td colspan="11">暂无远程备份设置.</td></tr>
	<?php	
	}
	else
	{
	...
	?>
			...
			<td><?php echo $val['remote_path'];?></td>
			<td><?php echo $val['remote_user'];?></td>
			<td><?php echo $val['remote_pass_type'] == '1' ? '密码' : '<font color="green">密匙</font>';?></td>
			<td>******</td>
			<td><?php echo !empty($val['remote_comment']) ? $val['remote_comment'] : '<i>无</i>';?></td>
			<td><?php echo $val['remote_time'];?></td>
			<td>
			<a href="index.php?c=index&a=backup&category=backup_remote&check=<?php echo $val['remote_id'];?>" class="button" onclick="return connect_check(this);"><span class="loop  icon"></span>连接测试</a>
			<a href="index.php?c=index&a=backup&category=backup_remote&edit=<?php echo $val['remote_id'];?>" class="button"><span class="pen icon"></span>编辑</a>
			<a href="index.php?c=index&a=backup&category=backup_remote&del=<?php echo $val['remote_id'];?>" class="button" onclick="return confirm('确认删除远程备份设置ID:<?php echo $val['remote_id'];?>?');"><span class="cross icon"></span>删除</a>
			</td>
			</tr>
	<?php
		}
...
<form action="index.php?c=index&a=backup&category=backup_remote" method="POST"  id="remote_edit" onsubmit="return remote_pass_type_fun(G('remote_pass_type_dom'));"/>
...
<?php if (isset($edit_remote)) { ?>
	<input type="hidden" name="save_eidt" value="<?php echo $_POST['remote_id'];?>" />
<?php } else { ?>
